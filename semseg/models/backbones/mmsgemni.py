...
class CustomDWConv(nn.Module):
    def __init__(self, dim, kernel):
        super().__init__()
        self.conv0_1 = nn.Conv2d(dim, dim, (1, kernel), padding=(0, int(kernel / 2)), groups=dim)
        self.conv0_2 = nn.Conv2d(dim, dim, (kernel, 1), padding=(int(kernel / 2), 0), groups=dim)

        init.kaiming_normal_(self.conv0_1.weight, mode='fan_in', nonlinearity='relu')
        init.kaiming_normal_(self.conv0_2.weight, mode='fan_in', nonlinearity='relu')
...
class MixTransformer(nn.Module):
    ...
    def init_weights(self):
        ...
        checkpoint = torch.load(f'/home/b311/data/zhangwei/MMSFormer/checkpoints/pretrained/segformer/mit_{self.model_name.lower()}.pth', map_location=torch.device('cpu'))
