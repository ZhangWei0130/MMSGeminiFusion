...
def main(cfg, save_dir, drop_path_rate):
    start = time.time()
    best_mIoU = 0.0
    best_epoch = 0
    num_workers = 2
    ...
    model = eval(model_cfg['NAME'])(model_cfg['BACKBONE'], trainset.n_classes, dataset_cfg['MODALS'], drop_path_rate)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg["AMP"]):
                if model_cfg["NAME"] == "MMSGeminiFusion":
                    loss = 0.0
        ...
        if ((epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 and (epoch+1)>train_cfg['EVAL_START']) or (epoch+1) == epochs:
            if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
                acc, macc, f1, mf1, ious, miou, test_loss = evaluate(cfg, model, valloader, device, loss_fn=loss_fn)
                ...
                if miou > best_mIoU:
                    prev_best_ckp = save_dir / f"{model_cfg['NAME']}_{train_cfg['PTH_NAME']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}_checkpoint.pth"
                    prev_best = save_dir / f"{model_cfg['NAME']}_{train_cfg['PTH_NAME']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}.pth"
                    if os.path.isfile(prev_best): os.remove(prev_best)
                    if os.path.isfile(prev_best_ckp): os.remove(prev_best_ckp)
                    best_mIoU = miou
                    best_epoch = epoch+1
                    cur_best_ckp = save_dir / f"{model_cfg['NAME']}_{train_cfg['PTH_NAME']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}_checkpoint.pth"
                    cur_best = save_dir / f"{model_cfg['NAME']}_{train_cfg['PTH_NAME']}_{dataset_cfg['NAME']}_epoch{best_epoch}_{best_mIoU}.pth"
...
if __name__ == '__main__':
    ...
    main(cfg, save_dir, args.drop_path_rate)
    cleanup_ddp()